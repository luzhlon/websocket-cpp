...
static void reset(uint32_t _digest[], std::string &buffer, uint64_t &transforms)
{
...
static void transform(uint32_t _digest[], uint32_t block[BLOCK_INTS], uint64_t &transforms)
{
...
SHA1::SHA1()
{
    ::reset(_digest, buffer, transforms);
}
...
void SHA1::update(std::istream &is)
{
    while (true)
    {
        ...
        transform(_digest, block, transforms);
        buffer.clear();
    }
}
...
std::string SHA1::digest() {
	...
	calc();
	if (is_little())
		for (auto i = 0; i < sizeof(_digest) / sizeof(_digest[0]); i++) {
			char *l = (char *)&_digest[i];
			char *p = buf + sizeof(*_digest) * i;
			p[0] = l[3], p[1] = l[2], p[2] = l[1], p[3] = l[0];
		}
	else
		memcpy(buf, _digest, 20);
	std::string str(buf, 20);
	return std::move(str);
}
